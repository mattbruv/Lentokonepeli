...
export function Scoreboard({ playerData, myPlayerGuid
}: ScoreboardProps) {

    const me = playerData.find(x => x.guid === myPlayerGuid)
    const myTeam = me?.team ?? "Centrals"

    const notUs = playerData
        .filter(x => x.team != myTeam)
            ...
            <Table verticalSpacing={2}>
                <Table.Thead>
                    <Table.Tr className="header">
                        <Table.Th><Center>Team</Center></Table.Th>
                        <Table.Th>Name</Table.Th>
                        <Table.Th><Center>Score</Center></Table.Th>
                        <Table.Th><Center>Frags</Center></Table.Th>
                        <Table.Th><Center>Deaths</Center></Table.Th>
                        <Table.Th><Center>Accuracy</Center></Table.Th>
                ...
                <Table.Tbody>
                    {sortedPlayers.map(player => (
                        <Table.Tr key={player.guid} className={getTeamClass(myTeam, player.team)}>
                            <Table.Td><Center><img src={getFlag(player.team)} /></Center></Table.Td>
                            <Table.Td>{formatName(player.name ?? "?", player.clan ?? null)}</Table.Td>
                            <Table.Td><Center>{player.score ?? 0}</Center></Table.Td>
                            <Table.Td><Center>{player.kills ?? 0}</Center></Table.Td>
                            <Table.Td><Center>{player.deaths ?? 0}</Center></Table.Td>
                            <Table.Td></Table.Td>
                            <Table.Td></Table.Td>
                        </Table.Tr>
                    ))}
